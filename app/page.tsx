...
export default function HomePage() {
  ...
  // ФИКСИРАНА ФУНКЦИЯ ЗА СКАНИРАНЕ
  const runAudit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
        ...
        <form onSubmit={runAudit}>
          ...
          <div style={{ display: 'flex', gap: '20px', marginBottom: '60px' }}>
            <button 
              ...
              style={{ flex: 1, height: '70px', background: 'white', color: 'black', border: 'none', borderRadius: '4px', fontWeight: '900', fontSize: '18px', letterSpacing: '2px', cursor: 'pointer', opacity: (isLoading || !input.trim()) ? 0.5 : 1 }}
            >
              {isLoading ? 'SCANNING_LOGIC...' : 'СТАРТИРАЙ СКАНИРАНЕ'}
            </button>
            <button type="button" onClick={clearAll} style={{ padding: '0 30px', background: 'transparent', border: '1px solid #333', color: '#666', borderRadius: '4px', cursor: 'pointer', fontSize: '12px', fontWeight: 'bold' }}>ИЗЧИСТИ</button>
          </div>
        </form>

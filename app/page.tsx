...
export default function HomePage() {
  ...
  const runAudit = async () => {
    ...
    try {
      ...
      setIsLoading(false)
      setInput('') // Изчистваме само полето за писане след старт
    }
  }
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', gap: '20px', marginBottom: '60px' }}>
          <button 
            ...
            style={{ flex: 3, height: '70px', background: 'white', color: 'black', border: 'none', borderRadius: '4px', fontWeight: '900', fontSize: '18px', letterSpacing: '2px', cursor: 'pointer', opacity: (isLoading || !input.trim()) ? 0.5 : 1 }}
          >
          ...
          <button 
            onClick={clearMemory}
            style={{ flex: 1, height: '70px', background: 'transparent', border: '1px solid #333', color: '#666', borderRadius: '4px', fontWeight: '900', fontSize: '14px', letterSpacing: '1px', cursor: 'pointer' }}
          >
            ИЗЧИСТИ
          </button>
        </div>
